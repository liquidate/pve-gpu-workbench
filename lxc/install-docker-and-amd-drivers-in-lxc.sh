...
echo ""
echo -e "${GREEN}Install Ollama (AI model runner):${NC}"
echo "  curl -fsSL https://ollama.com/install.sh | sh"
echo "  ollama run llama3.2:3b"
echo ""
echo -e "${GREEN}Or run Ollama in Docker:${NC}"
echo "  docker run -d --device /dev/kfd --device /dev/dri \\"
echo "    -e HSA_OVERRIDE_GFX_VERSION=11.5.1 -e HSA_ENABLE_SDMA=0 \\"
echo "    --group-add video --name ollama \\"
echo "    -v ollama:/root/.ollama -p 11434:11434 ollama/ollama"
echo "  docker exec -it ollama ollama run llama3.2:3b"
echo ""
echo -e "${YELLOW}Manual Docker GPU test command:${NC}"
echo "  docker run --rm --name rcom-smi --device /dev/kfd --device /dev/dri \\"
echo "    -e HSA_OVERRIDE_GFX_VERSION=11.5.1 -e HSA_ENABLE_SDMA=0 --group-add video \\"
echo "    --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --ipc=host \\"
echo "    rocm/rocm-terminal bash -c \"rocm-smi --showmemuse --showuse --showmeminfo all --showhw --showproductname\""
echo ""